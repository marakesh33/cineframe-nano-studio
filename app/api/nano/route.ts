...
export async function POST(request: Request) {
  try {
    ...
    const styleLockedPrompt = referenceImages.length
      ? `SCENE CONTENT — HIGHEST PRIORITY:\n${body.prompt}\n\nCAMERA GUIDANCE FOR FRAME ${sceneId}: use ${cameraDirection}. Keep neighboring frames visually connected. Change camera distance or angle only when it helps the story, while avoiding the exact same centered framing throughout the whole sequence.\n\nCONTENT RULES: Show the exact people, action, location, objects and symbolism requested in the scene. Do not replace it with an unrelated portrait or copy a reference composition. ${motifDirection} Other reference objects such as mirrors, green lamps, clothing racks and monitors may appear only when naturally useful to the requested scene and must never repeat as the central object. Keep locations and character positions coherent across connected scenes.\n\nSTRICT OLD SOFT VISUAL STYLE LOCK: Use the attached images only as references for painting technique, facial rendering, brushwork, light, palette, grain and emotional tone. Match the older deliberately soft and slightly mushy cinematic oil-painting look: broad loose brushstrokes, softened contours and facial detail, gentle optical blur, low microcontrast, smoky haze and glowing light bloom. Keep the perceived midtones approximately 15–20 percent brighter than a dark noir frame: lift teal-black shadow detail so faces, silhouettes and interiors remain clearly readable, preserve crimson-red and warm amber light, and never crush large areas into pure black. Retain an edge vignette, subtle analog grain and slight vintage compression texture. Preserve expressive emotions and readable silhouettes, but avoid crisp modern digital detail, photographic sharpness, glossy CGI, hyperreal skin, clean vector-like edges, neon colors and over-detailed backgrounds. The result should feel like a moody but readable hand-painted frame from the channel's older successful videos, not an underexposed image or a polished AI photograph. Never reproduce reference text, subtitles, logos, the words Psychology of Mind / Психология разума, or any watermark.`
      : body.prompt;
